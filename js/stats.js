...
const renderStats = ctx2d => {
   ...
   textY = renderBigStat('Total blocks produced or collected', Math.floor(Stats.lifetimeBlocksCollected).toLocaleString(), ctx2d, textY, maxWidth);
   textY = renderBigStat('Total pollutants produced', Math.floor(Data.lifetimePollution).toLocaleString(), ctx2d, textY, maxWidth);
   ...
   for (let i = 0; i < numStats; i++) {
      switch (i) {
         case 0:
            // ctx2d.fillText("Favorite color: " + Stats.favoriteColor, textX, toBrowserY(textY));
            break;
      }
